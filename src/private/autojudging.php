...
if ($ds == '/') {
    ...
    system
	("find $basdir -group users -exec chgrp root '{}' \\; 2>/dev/null");
    system
	("find $basdir -perm /1002 -type d > /tmp/boca.writabledirs.tmp 2>/dev/null");
...
while (42) {
	...
	if (($info =
	     @parse_ini_file($dir . $ds . "problemdata" . $ds . "description" . $ds . 'problem.info')) == = false) {
	...
	foreach(glob($dir.$ds."problemdata".$ds."limits".$ds.'*') as $file) {
		...
		DBGiveUpRunAutojudging($contest, $site, $number, $ip,
				       "Autojuging error: problem package file is invalid (5)");
		cleardir($dir.$ds."problemdata");
		continue;
	    }
	    $limits[$basename][basename($file)] =
		file('stdout',
	...
	foreach(glob($dir.$ds."problemdata".$ds."tests".$ds.'*') as $file) {
		...
		DBGiveUpRunAutojudging($contest, $site, $number, $ip,
				       "Autojuging error: internal test script failed (".
				       $file.")");
		$cont = true;
		break;
	    }
	...
	file_put_contents($cache.$ds.$run["inputoid"].".".
			  $run["inputname"], encryptData($s, $key));
    }

    if (!isset($limits[$basename][$run["extension"]][0])
	|| !is_numeric($limits[$basename][$run["extension"]][0])
	|| !isset($limits[$basename][$run["extension"]][1])
	|| !is_numeric($limits[$basename][$run["extension"]][1])
	|| !isset($limits[$basename][$run["extension"]][2])
	|| !is_numeric($limits[$basename][$run["extension"]][2])
	|| !isset($limits[$basename][$run["extension"]][3])
	|| !is_numeric($limits[$basename][$run["extension"]][3])) {
	echo "Failed to find proper limits information for the problem -- please check the problem package\n";
	DBGiveUpRunAutojudging($contest, $site, $number, $ip,
			       "Autojuging error: problem package file is invalid (6)");
	continue;
    }
// COMPILATION
//# parameters are:
...
//# $4 maximum allowed memory (in MBytes)
	...
	DBGiveUpRunAutojudging($contest, $site, $number, $ip,
			       "Autojuging error: problem package file is invalid (7)");
	continue;
    }

    $script = $dir.$ds.'compile'.$ds.$run["extension"];
    if (!is_file($script)) {
	echo "Error (not found) compile script for ".$run["extension"].
	    " -- please check the problem package\n";
	DBGiveUpRunAutojudging($contest, $site, $number, $ip,
			       "Autojuging error: compile script failed (".
			       $run["extension"].")");
	continue;
    }
    ...
    chmod($script, 0700);
    $ex =
	escapeshellcmd($script)." ".escapeshellarg($run["sourcename"]).
...
//# $6 maximum allowed output size (in KBytes)
	...
	if ($zip->open($dir.$ds.$run["inputname"]) == = true) {
		...
		if ($pos !== false
		    && $pos == strlen(dirname($filename)) - 5) {
		    $inputlist[$ninputlist++] =
			'input'.$ds.basename($filename);
		    $outputlist[$noutputlist++] =
			'output'.$ds.basename($filename, '.link');
		}
	    }
	    $zip->extractTo($dir,
			    array_merge(array("run".$ds.$run["extension"]),
					array("compare".$ds.
					      $run["extension"]),
					$inputlist, $outputlist));
	    $zip->close();
	    if (chmod($dir.$ds.'output', 0700) == false
		|| chown($dir.$ds.'output', 'root') == false) {
		echo "Failed to chown/chdir the output folder -- please check the system and problem package\n";
		DBGiveUpRunAutojudging($contest, $site, $number, $ip,
				       "Autojuging error: chown/chmod failed for output (99)");
		continue;
	    }
	    if (chmod($dir.$ds.'compare', 0700) == false
		|| chown($dir.$ds.'compare', 'root') == false) {
		echo "Failed to chown/chdir the output folder -- please check the system and problem package\n";
		DBGiveUpRunAutojudging($contest, $site, $number, $ip,
				       "Autojuging error: chown/chmod failed for output (99)");
		continue;
	    ...
	    DBGiveUpRunAutojudging($contest, $site, $number, $ip,
				   "Autojuging error: problem package file is invalid (8)");
	    continue;
	}
	$retval = 0;
	...
	if (!is_file($script)) {
	    ...
	    DBGiveUpRunAutojudging($contest, $site, $number, $ip,
				   "Autojuging error: problem package file is invalid (9)");
	    continue;
	}
	chdir($dir);
	chmod($script, 0700);
	...
	if ($ninputlist == 0) {
	    ...
	    DBGiveUpRunAutojudging($contest, $site, $number, $ip,
				   "Autojuging error: problem package file is invalid (10)");
	    continue;
	} else {
	    $errp = 0;
		...
		if (is_file($dir.$ds."input".$ds.$file)) {
			...
			if (is_readable($fnam)) {
			    ...
			    DBGiveUpRunAutojudging($contest, $site,
						   $number, $ip,
						   "Autojuging error: problem package file is invalid (11) or missing files on the autojudge");
			    $errp = 1;
			    break;
			}
		    }
		    ...
		    system('/bin/echo ##### STDERR FOR FILE '.
			   escapeshellarg($file).' >> '.$dir.$ds.'allerr');
		    system('/bin/cat stderr >> '.$dir.$ds.'allerr');
		    system('/bin/cat stdout > '.$dir.$ds.'team'.$ds.
			   escapeshellarg($file));
		    system('/bin/echo ##### STDOUT FOR FILE '.
			   escapeshellarg($file).' >> '.$dir.$ds.'allout');
			...
			$ex =
			    escapeshellcmd($scriptcomp).
			    " ".escapeshellarg($dir.$ds."team".$ds.$file).
			    " ".escapeshellarg($dir.$ds."output".$ds.
					       $file).
			    " ".escapeshellarg($dir.$ds."input".$ds.$file).
			    " >compout";
			echo "Executing ".$ex." at ".getcwd().
			    " for output file $file\n";
			if (system($ex, $localretval) == = false)
			    $localretval = -1;

			$fp = fopen($dir.$ds."allerr", "a+");
			fwrite($fp,
			       "\n\n===OUTPUT OF COMPARING SCRIPT FOLLOWS FOR FILE ".
			       $file." (EMPTY MEANS NO DIFF)===\n");
			$dif = file($dir.$ds."compout");
			$difi = 0;
			for (; $difi < count($dif) - 1 && $difi < 5000;
			     $difi++)
			    fwrite($fp, $dif[$difi]);
			if ($difi >= 5000)
			    fwrite($fp,
				   "===OUTPUT OF COMPARING SCRIPT TOO LONG - TRUNCATED===\n");
			else
			    fwrite($fp,
				   "===OUTPUT OF COMPARING SCRIPT ENDS HERE===\n");
			...
			echo "==> ERROR reading output file ".$dir.$ds.
			    'output'.$ds.$file." - skipping it!\n";
		    }

		} else {
		    echo "==> ERROR reading input file ".$dir.$ds."input".
			$ds.$file." - skipping it!\n";
		}
	    }
	    if ($errp == 1)
